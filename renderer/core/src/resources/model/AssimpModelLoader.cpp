...
namespace blitz
{
    ...
    Model* AssimpModelLoader::loadModel(const aiNode* modelNode, const aiScene* scene)
    {
        ...
        for (int meshIdx = 0; modelNode->mNumMeshes; ++meshIdx)
        {
        ...
        for (int meshIdx = 0; modelNode->mNumMeshes; ++meshIdx)
        {
            ...
            for (int vertexIdx = 0; vertexIdx < mesh->mNumVertices; ++vertexIdx)
            {
                ...
                memcpy(vertexData + offsetInElementData + (ASSIMP_3D_VECTOR_SIZE * 2), &mesh->mTextureCoords[0][vertexIdx], ASSIMP_2D_VECTOR_SIZE);
        ...
        vertexArray->addAttribute({ vertexBuffer, NORMALS, blitz::DataType::FLOAT, 3, false, 8 * sizeof(float), 3 * sizeof(float), 0 });
        vertexArray->addAttribute({ vertexBuffer, TEXTURE_COORDS, blitz::DataType::FLOAT, 2, false, 8 * sizeof(float), 6 * sizeof(float), 0 });

        if (elementData != nullptr)
        {
            auto elementBuffer = context->createBuffer({ elementBufferSize, blitz::UsageHint::STATIC, 0, blitz::BindHint::INDEX, vertexData, false, true });
            vertexArray->bindElementBuffer(elementBuffer, DataType::INT);
        }

        for (int childIdx = 0; childIdx < modelNode->mNumChildren; ++childIdx)
        {
    ...
    TextureSampler* AssimpModelLoader::loadMaterialTexture(Model* model, const aiMaterial* mat, const aiTextureType& type) const
    {
        ...
        char* path = new char[textureFilePath.length + 1];
        strncpy(path, textureFilePath.data, textureFilePath.length);
        path[textureFilePath.length] = '\0';

        auto textureLoader = new STBImage2DTextureLoader({nullptr, path, 0});
